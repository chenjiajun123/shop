<template>
	<div class="productlist-one mgt30">
		<ul class="disflex justifyBetween flex-wrap mgb15">
			<li class="disflex  productList-li mgt30 column " v-for="(item,index,key) in showlist" :key="item.id" :index="index">
				...
				<div class="relative disflex alignCenter mgt15 justifyBetween"><span class="font12 col-sred ">￥{{item.number}}</span>
					<router-link :to="{ path:'/productDetails:id', query:{ id:item.id,tabId:1}}" class="buy-once font14">立即购买</router-link>
				</div>
			</li>
		</ul>
	...
	export default {
		...
		computed: {
			showlist: function(){
				if(this.showAll == false) {
					let showlist = [];
					if(this.productListOne.length > 4) {
						for(let i = 0; i < 4; i++) {
							showlist.push(this.productListOne[i])
						}

					} else {
						showlist = this.productListOne
					}
					return showlist
				} else {
		...
		activated() {
			if(!this.$route.meta.isBack) {
				...
				this.$ajax.get('/productListOne').then((res) => {

					this.productListOne = res.data[0].productListOne
